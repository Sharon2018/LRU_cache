...
The order in the Queue representes the order of "used time". e.g., the Head indicates the most recently used item, and the tail incdicates the least recently used item. We do "remove" and "insert" to keep the order updated.  The time conplexity of get(key) for a queue is O(n). 
